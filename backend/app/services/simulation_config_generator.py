...
class SimulationConfigGenerator:
    ...
    def __init__(
        ...
        self.boost_llm = LLMClient(api_key=api_key, model=model_name, use_boost=True)
    ...
    def generate_config(
        ...
        profile_stance_map: Optional[Dict[str, Dict[str, Any]]] = None,
        ...
        Args:
            ...
            profile_stance_map: 프로필에서 추출한 stance/sentiment_bias 매핑 {entity_uuid: {stance, sentiment_bias}}
        ...
        # ========== 단계1+2: 시간 설정 + 이벤트 설정 병렬 생성 ==========
        report_progress(1, "시간 설정 + 이벤트 설정 병렬 생성 중...")
        num_entities = len(entities)

        with ThreadPoolExecutor(max_workers=2) as pool:
            future_time = pool.submit(self._generate_time_config, context, num_entities)
            future_event = pool.submit(
                self._generate_event_config, context, simulation_requirement, entities,
                use_boost=True
            )

            time_config_result = future_time.result(timeout=180)
            event_config_result = future_event.result(timeout=180)

        time_config = self._parse_time_config(time_config_result, num_entities)
        reasoning_parts.append(f"시간 설정: {time_config_result.get('reasoning', '성공')}")

        report_progress(2, "이벤트 설정 파싱 완료")
        event_config = self._parse_event_config(event_config_result)
        reasoning_parts.append(f"이벤트 설정: {event_config_result.get('reasoning', '성공')}")

        # ========== 단계3-N: 배치별 Agent 설정 병렬 생성 ==========
        all_agent_configs = []
        max_batch_workers = min(num_batches, 5)

        if num_batches <= 1:
            # 배치가 1개면 병렬화 불필요
            batch_entities = entities[:self.AGENTS_PER_BATCH]
            report_progress(3, f"Agent 설정 생성 중 (1-{len(batch_entities)}/{len(entities)})...")
            ...
            report_progress(3, f"Agent 설정 {num_batches}개 배치 병렬 생성 중...")
            batch_futures = {}
            with ThreadPoolExecutor(max_workers=max_batch_workers) as pool:
                for batch_idx in range(num_batches):
                    ...
                    batch_futures[future] = (batch_idx, start_idx, end_idx)

                # start_idx 기준으로 정렬하여 순서 보장
                results_by_start = {}
                for future in as_completed(batch_futures):
                    batch_idx, start_idx, end_idx = batch_futures[future]
                    try:
                        batch_configs = future.result(timeout=180)
                        results_by_start[start_idx] = batch_configs
                        logger.info(f"Agent 배치 {batch_idx + 1}/{num_batches} 완료 ({start_idx + 1}-{end_idx})")
                    except Exception as e:
                        logger.error(f"Agent 배치 {batch_idx + 1} 실패: {e}")
                        results_by_start[start_idx] = []

                for start_idx in sorted(results_by_start.keys()):
                    all_agent_configs.extend(results_by_start[start_idx])
    ...
    def _call_llm_with_retry(self, prompt: str, system_prompt: str, use_boost: bool = False) -> Dict[str, Any]:
        ...
        llm_client = self.boost_llm if use_boost else self.llm
        content, finish_reason = llm_client.chat_with_retry(
            messages=messages,
            temperature=0.7,
...
- reasoning (string): 이렇게 설정한 이유 간략 설명"""
    ...
    def _generate_event_config(
        ...
        """이벤트 설정 생성 (use_boost=True이면 Boost LLM 사용)"""
...
{{
    ...
    "reasoning": "<간략한 설명>"
        ...
        try:
            return self._call_llm_with_retry(prompt, system_prompt, use_boost=use_boost)
        except Exception as e:
    ...
    def _parse_event_config(self, result: Dict[str, Any]) -> EventConfig:
        ...
        return EventConfig(
            ...
            scheduled_events=[],
            hot_topics=_normalize_topics(result.get("hot_topics", [])),
    ...
    def _generate_agent_configs_batch(
        ...
        """배치별 Agent 설정 생성 (use_boost=True이면 Boost LLM 사용)"""
        ...
        for i, e in enumerate(entities):
            ...
            # 프로필에서 이미 결정된 stance/sentiment_bias가 있으면 포함
            if profile_stance_map and e.uuid in profile_stance_map:
                stance_data = profile_stance_map[e.uuid]
                if stance_data.get("stance"):
                    entry["preset_stance"] = stance_data["stance"]
                if stance_data.get("sentiment_bias") is not None:
                    entry["preset_sentiment_bias"] = stance_data["sentiment_bias"]
...
**중요**: 엔티티에 preset_stance 또는 preset_sentiment_bias가 제공된 경우, 반드시 해당 값을 stance와 sentiment_bias로 사용하세요. 이 값은 페르소나 생성 단계에서 이미 결정된 것이며 정합성 유지를 위해 변경하면 안 됩니다.
...
{{
    "agent_configs": [
        ...
        try:
            result = self._call_llm_with_retry(prompt, system_prompt, use_boost=use_boost)
        ...
        for i, entity in enumerate(entities):
            ...
            # 프로필에서 이미 결정된 stance/sentiment_bias 우선 적용
            entity_data = profile_stance_map.get(entity.uuid, {}) if profile_stance_map else {}
            stance = entity_data.get("stance") or cfg.get("stance", "neutral")
            sentiment_bias = entity_data.get("sentiment_bias")
