      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            ...
            <div class="action-buttons">
                ...
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? '전체 화면 종료' : '전체 화면'">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">그래프 데이터 로드 중...</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">생성 완료 후 자동으로 그래프 구축 시작</p>
          </div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  문서 업로드 후, LLM이 문서 내용을 분석하고, 여론 시뮬레이션에 적합한 온톨로지 구조를 자동 생성 (엔티티 유형 + 관계 유형)
                </div>
              </div>
              
              <!-- 온톨로지 생성진행 상황 -->
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">생성진행 상황</div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">온톨로지 생성 대기...</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API 설명</div>
                <div class="detail-content">
                  생성된 온톨로지를 기반으로 문서를 청킹한 후 Zep API를 호출하여 지식 그래프를 구축하고, 엔티티와 관계를 추출
                </div>
              </div>
              
              <!-- 온톨로지 완료 대기 -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">온톨로지 생성 대기완료...</div>
              </div>
              
              <!-- 구축 진행 상황 -->
              <div class="detail-section" v-if="buildProgress && currentPhase >= 1">
                <div class="detail-label">구축 진행 상황</div>
...
const ontologyProgress = ref(null) // 온톨로지 생성진행 상황
...
const handleNewProject = async () => {
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // 온톨로지 생성단계
...
const startBuildGraph = async () => {
  try {
    ...
    error.value = '시작그래프 구축실패: ' + (err.message || '알 수 없는 오류')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ 그래프 구축완료，진행 중로드완전한 데이터...')
        ...
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 로드완전한 그래프:', projectResponse.data.graph_id)
...
const renderGraph = () => {
  ...
  simulation.on('tick', () => {
    // 모든 엣지 위치 업데이트 (가시적 라인과 투명 클릭 영역 포함)
