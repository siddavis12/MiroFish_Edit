            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">진행 중생성{{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">보고서 생성 에이전트의 빠른 대화 버전으로 4가지 전문 도구를 호출할 수 있으며, MiroFish의 완전한 기억을 보유</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">현실 세계 시드 데이터와 시뮬레이션 환경 상태를 정렬하고, Global/Local Memory 메커니즘을 결합하여 시공간을 초월한 심층 귀인 분석 제공</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">PanoramaSearch 전경 추적</div>
                    <div class="tool-desc">그래프 구조 기반 너비 우선 탐색 알고리즘으로 이벤트 전파 경로를 재구성하고, 전체 정보 흐름의 토폴로지 구조를 캡처</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">GraphRAG 기반 즉시 쿼리 인터페이스로 인덱싱 효율성을 최적화하여 구체적인 노드 속성과 이산 사실을 빠르게 추출</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">자율적 인터뷰로 시뮬레이션 세계의 개체들과 병렬로 다회 대화를 수행하여 비정형 관점 데이터와 심리 상태를 수집</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Report Agent와 대화하여 보고서 내용 심층 이해' : '시뮬레이션 개체와 대화하여 관점 파악' }}
              </p>
            </div>
            <div 
          ...
          <div class="survey-setup">
            <div class="setup-section">
              ...
              <div class="selection-actions">
                ...
                <button class="action-link" @click="clearAgentSelection">초기화</button>
              </div>
            </div>
...
const renderMarkdown = (content) => {
  ...
  // 블록 요소 뒤 바로 오는 단락 시작 태그 앞의 <br> 정리
...
const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error('시뮬레이션 개체를 먼저 선택하세요')
  }
  ...
  if (res.success && res.data) {
    ...
    // 객체 딕셔너리를 배열로 변환, reddit 플랫폼 응답 우선 가져오기
    let responseContent = null
    const agentId = selectedAgentIndex.value
    
    if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
      // reddit 플랫폼 응답 우선 사용, 그 다음 twitter
    ...
    if (responseContent) {
      ...
      addLog(`${selectedAgent.value.username} 응답 완료`)
    } else {
      throw new Error('응답 없음데이터')
    }
  } else {
    throw new Error(res.error || '요청 실패')
  }
...
const submitSurvey = async () => {
  ...
  addLog(`${selectedAgents.value.size}명에게 설문 전송...`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`수신됨 ${surveyResults.value.length} 개 응답`)
    } else {
      throw new Error(res.error || '요청 실패')
    }
  } catch (err) {
    addLog(`설문전송 실패: ${err.message}`)
  } finally {
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`로드 완료: ${profiles.value.length} 개 시뮬레이션 개체`)
    }
  } catch (err) {
...
/* Left Panel - Report Style (Step4Report.vue와 완전 동일) */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
/* 순서 목록 번호 수정 - CSS 카운터를 사용하여 여러 ol의 연속 번호 매기기 */
.message-text {
