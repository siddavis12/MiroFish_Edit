            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">생성 중: {{ section.title }}...</span>
                </div>
              </div>
            </div>
          </div>
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (콘텐츠 생성 완료, 전체 장 생성은 아직 미완료일 수 있음) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // DOM 업데이트 후 스크롤 위치 조정하여 버튼이 동일 위치에 유지되도록
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // 인터뷰 대상 선정 이유 추출
    const reasonMatch = text.match(/### 인터뷰 대상 선정 사유\n([\s\S]*?)(?=\n---\n|\n### 인터뷰 실록)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // 형식1: 숫자. **이름(index=X)**: 사유
        // 예: 1. **동문_345(index=1)**: 동문으로서...
        ...
        // 형식2: - 선택 이름(index X): 사유
        // 예: - 선택 학부모_601(index 0): 학부모 대표로서...
        if (!headerMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // 응답 추출 - Twitter와 Reddit 분리
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*핵심 인용|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Twitter와 Reddit 응답 분리
        const twitterMatch = answerText.match(/【Twitter 플랫폼 응답】\n?([\s\S]*?)(?=【Reddit 플랫폼 응답】|$)/)
        const redditMatch = answerText.match(/【Reddit 플랫폼 응답】\n?([\s\S]*?)$/)
        ...
        // 플랫폼 폴백 로직 (이전 형식 호환: 하나의 플랫폼 마커만 있는 경우)
        if (!twitterMatch && redditMatch) {
          // Reddit 응답만 있음, 비플레이스홀더 텍스트일 때만 기본 표시로 복사
          if (interview.redditAnswer && interview.redditAnswer !== '(해당 플랫폼에서 응답을 받지 못함)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(해당 플랫폼에서 응답을 받지 못함)') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // 플랫폼 마커 없음 (매우 오래된 형식), 전체를 응답으로 사용
          interview.twitterAnswer = answerText
        }
      }
      
      // 핵심 인용 추출 (다양한 인용부호 형식 호환)
      const quotesMatch = block.match(/\*\*핵심 인용:\*\*\n([\s\S]*?)(?=\n---|\n####|$)/)
      if (quotesMatch) {
        const quotesText = quotesMatch[1]
        // > "text" 형식 우선 매칭
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // 폴백: > "text" 또는 > \u201Ctext\u201D (동아시아 인용부호) 매칭
        if (!quoteMatches) {
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `히스토리 기억 (${props.result.historicalFacts.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, '히스토리 기억'),
          ...
          ) : h('div', { class: 'empty-state' }, '히스토리 기억 없음'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // 각 질문-응답 쌍에 대해 독립적인 플랫폼 선택 상태 유지
    ...
    // 플랫폼 플레이스홀더 텍스트 여부 확인
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(해당 플랫폼에서 응답을 받지 못함)' || t === '[응답 없음]'
    }
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // 분할 실패: 첫 번째 질문은 전체 응답 반환, 나머지는 빈 문자열 반환
      return qIdx === 0 ? answer : ''
    }
    
    // 특정 질문에 듀얼 플랫폼 응답이 있는지 확인 (플레이스홀더 텍스트 필터링)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // 두 플랫폼 모두 실제 응답이 있고 (플레이스홀더 아님) 콘텐츠가 다름
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // 듀얼 플랫폼 전환 버튼 (실제 듀얼 플랫폼 응답이 있을 때만 표시)
                  ...
                  // Expand/Collapse Button (플레이스홀더 텍스트는 표시하지 않음)
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, '관련 결과를 찾을 수 없음'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `접기 ▲` : `전체 ${props.result.facts.length}건 펼치기 ▼`)
...
// renderMarkdown은 '../utils/markdown'에서 import
...
const fetchAgentLog = async () => {
  ...
  try {
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // loading 상태 삭제 보장
            emit('update-status', 'completed')
            stopPolling()
            // 스크롤 로직은 루프 종료 후 nextTick에서 통합 처리
          }

          // 에러 발생 시에도 완료 처리하여 UI가 멈추지 않도록 함
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // 작업 완료 시 상단으로 스크롤; 그렇지 않으면 하단으로 스크롤하여 최신 로그 따라가기
            if (isComplete.value) {
    ...
    // 보고서가 이미 완료 상태인데 report_complete 로그가 없는 경우 강제 완료
    if (waitingForComplete.value && !isComplete.value) {
      // 더 이상 새 로그가 없으면 (모든 로그를 읽은 상태) 완료 처리
...
// 최종 답변 콘텐츠 추출 - LLM response에서 섹션 콘텐츠 추출
const extractFinalContent = (response) => {
  if (!response) return null
  
  // <final_answer> 태그 내의 콘텐츠 추출 시도
  ...
  // Final Answer: 뒤의 콘텐츠 찾기 시도 (다양한 형식 지원)
  // 형식1: Final Answer:\n\n콘텐츠
  // 형식2: Final Answer: 콘텐츠
  ...
  // ## 또는 # 또는 >로 시작하면 직접적인 markdown 콘텐츠일 수 있음
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // 콘텐츠가 길고 markdown 형식을 포함하면, 사고 과정 제거 후 반환 시도
...
// 보고서가 이미 완료 상태인지 확인하여 isComplete 복원
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (status === 'completed' || status === 'COMPLETED') {
        // 이미 완료된 보고서 — 로그 폴링으로 섹션 내용 복원 후 isComplete 설정
        // 로그 폴링이 한 번 돌아서 섹션을 채운 뒤 완료 표시
