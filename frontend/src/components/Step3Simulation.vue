          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- 범용 폴백: 알 수 없는 유형이거나 content가 있지만 위에서 처리되지 않음 -->
...
const runStatus = ref({})
const allActions = ref([]) // 모든 동작 (증분 누적)
const actionIds = ref(new Set()) // 중복 제거용 동작 ID 세트
const scrollContainer = ref(null)

// Computed
// 시간순으로 동작 표시 (최신 항목이 마지막, 즉 하단)
...
const resetAllState = () => {
  ...
  stopPolling()  // 이전에 존재할 수 있는 폴링 정지
}
...
const doStartSimulation = async () => {
  ...
  // 모든 상태를 먼저 리셋하여 이전 시뮬레이션의 영향을 받지 않도록 보장
  resetAllState()
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`정지 실패: ${res.error || '알 수 없는 오류'}`)
    }
  } catch (err) {
    addLog(`정지 오류: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // 추가 확인: 백엔드가 아직 runner_status를 업데이트하지 않았지만 플랫폼이 이미 완료를 보고한 경우
      // twitter_completed와 reddit_completed 상태를 감지하여 판단
...
const checkPlatformsCompleted = (data) => {
  ...
  // 최소 하나의 플랫폼이 완료되면 모든 활성화된 플랫폼이 완료되었는지 확인
  // actions_count로 플랫폼 활성화 여부 판단 (count > 0이거나 running이 true였던 경우)
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // all_actions를 사용하여 전체 동작 목록 가져오기
      ...
      // 자동 스크롤하지 않아 사용자가 자유롭게 타임라인 확인 가능
      // 새 동작은 하단에 추가
    }
  } catch (err) {
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('보고서 생성 요청 전송 완료, 잠시 기다려주세요...')
    return
  }
